...
pokemon.post("/", async (req, res, next) => {
    ...
    if (pok_name && pok_height && pok_weight && pok_base_experience) {
        ...
        if(rows.affectedRows == 1) {
            return res.status(201).json({ code : 200, message : 'Pokemon insertado correctamente' });
        }
...
pokemon.get('/:id([0-9]{1,3})', async (req, res, next) => {
    ...
    const pkid = await db.query("SELECT * FROM pokemon WHERE pok_id="+id);
    if (id >= 1 && id <= 722) {
        return res.status(201).json({code:201, message: pkid});
    } 
